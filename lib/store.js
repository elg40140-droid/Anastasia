...
if(!chats)chats=conn.chats[id]={...contact,id:id};
conn.chats[id]={...chats,...{...contact,id:id,...(id.endsWith("@g.us")?{subject:contact.subject||contact.name||chats.subject||""}:{name:contact.notify||contact.name||chats.name||chats.notify||""})||{}}};
}
...
let chats=conn.chats[id];
if(!chats)chats=conn.chats[id]={id:id};
...
writeFileSync(filename,JSON.stringify({creds:creds,keys:keys},BufferJSON.replacer,2));
saveCount=0;
}
};
...
export default{bind:bind,useSingleFileAuthState:useSingleFileAuthState,loadMessage:loadMessage};